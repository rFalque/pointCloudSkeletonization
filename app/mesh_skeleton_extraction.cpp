...
#include "skeleton_toolbox/meshSkeletonization.hpp"
...
int main(int argc, char* argv[])
{
    ...
    if (F_tmp.rows() == 0) {
        std::cout << "Error: the faces are empty (the input file is not a mesh)";
        return 0;
    }
    ...
    std::cout << "Progress: start skeletonization ... \n";
    MeshSkeletonization skeletonizer(V, F, opts);
    skeletonizer.skeletonize();
    std::cout << "Progress: skeletonization finished\n";
